    ...
    with open(kifu_list_file, 'r')as f:
        for line in tqdm(f.readlines()):
            ...
            for i in k:
                ...
                legal = i[129:].reshape((1,64))
                banlist = np.append(banlist, ban, axis=0)
                tebanlist = np.append(tebanlist, teban, axis=0)
                legallist = np.append(legallist, legal, axis=0)
            cnt += 1
            if cnt % 50 == 0:
                banlists = np.append(banlists, banlist, axis=0)
                tebanlists = np.append(tebanlists, tebanlist, axis=0)
                legallists = np.append(legallists, legallist, axis=0)
                banlist = np.empty((0, 2, 8, 8))
                tebanlist = np.empty((0, 1))
                legallist = np.empty((0,64))
        legallists_split = []
        legallists_split = np.split(legallists, 64, axis = 1)
        print(f'{kifu_list_file}を読み込みました')
    return banlists, tebanlists, legallists_split

if __name__ == '__main__':
    b_train, t_train, l_train = readkifu('kifulist_train.txt')
    b_test, t_test, l_test = readkifu('kifulist_test.txt')
    if os.path.isfile('model_legal.h5'):
        model = load_model('model_legal.h5')
        print('モデルmodel_legal.h5を読み込みました')
    else:
        print('モデルはありません')
        ...
        legal_outputs = []
        for _ in range(64):
            legal_outputs.append(Dense(1, activation='sigmoid')
                                 (dens_2))
    ...
    early_stopping = EarlyStopping(monitor='val_loss',
                                   patience=3,
                                   verbose=1)
    md_ch = ModelCheckpoint(filepath = 'model_legal.h5')
    history = model.fit([b_train, t_train], l_train,
                        epochs=20, batch_size=64,
                        validation_split=0.1, verbose=1,
                        callbacks=[early_stopping, md_ch])
    score = model.evaluate([b_test, t_test], l_test, verbose=0)
